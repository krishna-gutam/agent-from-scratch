...
def run_command(command):
    try:
        ...
        # Truncate output to 2000 characters to save tokens
        if len(output) > 2000:
            return output[:2000] + "\n... (output truncated)"
        return output
    except Exception as e:
...
TOOLS = [
    {
        "function_declarations": [
            ...
            {
                "name": "apply_patch",
                "description": "Replaces old_code with new_code using a multi-strategy fuzzy matching chain.",
                "parameters": {
                    "type": "OBJECT",
                    "properties": {
                        "file_path": {"type": "STRING", "description": "The path to the file."},
                        "old_code":{"type": "STRING","description": "The existing code snippet to be replaced."},
                        "new_code":{"type": "STRING","description": "The new code snippet that will replace the matched `old_code`."},
                    },
            ...
            },
            {
                ...
                "parameters": {
                    "type": "OBJECT",
                    "properties": {
                        "command": {"type": "STRING", "description": "The command to execute in powershell."}
                    },
                    "required": ["command"]
                }
            }
